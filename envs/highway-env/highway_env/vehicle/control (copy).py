...
class ControlledVehicle(Vehicle):
    ...
    def low_level_act(self, low_level_action=None):

        action = {'steering': low_level_action[0]*self.MAX_STEERING_ANGLE,
                  'acceleration': low_level_action[1]*self.DELTA_VELOCITY}
        print('controlled Vehicle MDP low level check ', action)
        super(ControlledVehicle, self).act(action)
...
class MDPVehicle(ControlledVehicle):
    ...
    def act(self, low_level=False, action=None):
        """
        ...
        if not low_level:
            ...
            print('MDP Vehicle MDP low level check ', action)
            super(MDPVehicle, self).low_level_act(low_level_action=action)
