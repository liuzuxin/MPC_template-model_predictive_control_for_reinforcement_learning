    ...
    def _simulate(self, action=None):
        ...
        for k in range(int(self.SIMULATION_FREQUENCY // self.config["policy_frequency"])):
            if action is not None and \
                ...
                if not self.config["low_level"]:
                    self.vehicle.act(low_level=self.config["low_level"], action=self.ACTIONS[action])
                else:
                    print('simulate Check ', k, 'action ', action)
                    self.vehicle.act(low_level=self.config["low_level"], action=action)
